...
const RoomCard = ({ room, onDelete, onClick, currentUserId }: RoomCardProps) => {
  ...
  return (
    <article
      onClick={onClick}
      className={`group  cursor-pointer relative flex min-h-60 flex-col overflow-hidden rounded-3xl border border-border bg-surface p-5 shadow-sm transition-all duration-200 ${
        onClick
      ...
      <div className="mt-5 flex items-end justify-between border-t border-border pt-4">
        <div>
          <div className="flex -space-x-2">
            {membersToShow.map((member) => {
              ...
              return (
                <div
                  key={member._id}
                  title={memberName}
                  className="flex h-7 w-7 items-center justify-center overflow-hidden rounded-full border-2 border-surface bg-primary-light text-[10px] font-semibold text-primary"
                >
                  {profilePic ? (
                    <img
                      src={profilePic}
            ...
            {remainingMembers > 0 && (
              <div className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-surface bg-surface-2 text-[10px] font-semibold text-text-secondary">
                +{remainingMembers}
              </div>
            )}
          </div>
